...
export class TeaCalendarioPorProfissionaisComponent implements OnInit {
  ...
  allProfessionals: Profissional[] = []; // Lista completa (não utilizada para filtro do calendário mensal)

  // Profissionais vindos da agenda compartilhada (mesmos do calendário mensal)
  ...
  ngOnInit(): void {
    // Carrega profissionais para o seletor
  ...
  onProfessionalChange(value: string): void {
    ...
    this.generateSidebarCalendar(); // Atualiza o calendário lateral com base no profissional selecionado
  }
  ...
  clearProfessionalFilters(): void {
    ...
    this.sessionSearch = '';
    this.selectedProfessional = '';
    this.initializeCalendar(); // Reinicializa o calendário para mostrar todos os profissionais
  }
  ...
  // Nome do profissional selecionado (quando ID está selecionado)
